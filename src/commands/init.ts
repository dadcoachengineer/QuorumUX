/**
 * `quorum init` — Interactive Project Setup Wizard
 *
 * Walks the user through configuring a new project and generates quorum.config.ts.
 */
...
export async function runInit(): Promise<void> {
  ...
  try {
    ...
    const configPath = path.join(process.cwd(), 'quorum.config.ts');

    if (fs.existsSync(configPath)) {
      const overwrite = await confirm('quorum.config.ts already exists. Overwrite?', false);
      if (!overwrite) {
    ...
    console.log('\n  Config written to quorum.config.ts');
    ...
    console.log(`    2. Run: npx quorum --dry-run`);
    console.log(`    3. Run: npx quorum\n`);
  } finally {
    closePrompt();
  }
}
...
async function resolveApiKeyStep(): Promise<string> {
  ...
  const saveChoice = await select('Where should the key be stored?', [
    {
      label: 'Global config (~/.quorum/config.json)',
...
function generateConfig(input: ConfigInput): string {
  ...
  return `import type { QuorumConfig } from './src/types';

/**
 * ${input.name} — Quorum Configuration
 *
