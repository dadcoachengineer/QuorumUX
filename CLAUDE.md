...
- **Title normalization** — `normalizeTitle()` in `compare.ts` strips severity prefixes (`[P0]`, `P1:`), removes filler adverbs, normalizes synonyms (`latency`→`performance`, `frozen`→`block`, `navigation`→`nav`), collapses whitespace. Applied inside `jaccardSimilarity()` before tokenization.
- **Persisting variants** — `PersistingVariant` in `compare.ts` captures issues that appear "resolved" and "new" but are actually reworded versions of the same issue (Jaccard ≥0.5 after normalization, below primary match threshold). Bidirectional detection with greedy best-match assignment.
