...
- **Title normalization** — `normalizeTitle()` in `compare.ts` strips severity prefixes (`[P0]`, `P1:`), removes filler adverbs, normalizes synonyms (generic: `latency`→`performance`, `frozen`→`block`, `navigation`→`nav`, `obscures`→`overlaps`, `exceeds`/`delay`→`slow`, `insufficient`→`missing`, `tracking`→`analytics`; domain-aware: `stepper`→`indicator`, `progress`→`step`), collapses whitespace. Applied inside `jaccardSimilarity()` before tokenization.
- **Persisting variants** — `PersistingVariant` in `compare.ts` captures issues that appear "resolved" and "new" but are actually reworded versions of the same issue (Jaccard ≥0.35 after normalization, below primary match threshold). Bidirectional detection with greedy best-match assignment. Threshold configurable via `--variant-threshold` CLI flag or `CompareOptions.variantThreshold`.
- **Score context** — `generateScoreContext()` in `compare.ts` produces an interpretive summary line: resolved P0s, new P0s, regressions, reworded variants.
- **Synthesis title convention** — Rule 8 in the synthesis prompt instructs models to use `[Component] Verb phrase` format for issue titles, improving cross-run matching stability.
